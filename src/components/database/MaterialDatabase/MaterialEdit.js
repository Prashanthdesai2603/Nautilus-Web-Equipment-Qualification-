...
const MaterialEdit = () => {
  ...
  const SaveData = () => {
    const StoredMaterialData = JSON.parse(sessionStorage.getItem("MaterialData")) || [];
    const updatedData = StoredMaterialData.map((m) =>
      m.id === SelectedRowId ? { ...SelectedMaterialData, id: SelectedRowId } : m
    );

    sessionStorage.setItem("MaterialData", JSON.stringify(updatedData));
  ...
  const CheckForDuplicateMaterialId = () => {
    return new Promise((resolve) => {
      ...
      const exists = StoredMaterialData.some(
        (m) =>
          m.id !== SelectedRowId &&
          String(m.Material_Id).toLowerCase() === String(SelectedMaterialData["Material_Id"]).toLowerCase()
      );
  ...
  const SubmitData = () => {
    CheckForMaterialId().then(() => {
      if (SelectedMaterialId !== SelectedMaterialData["Material_Id"]) {
        CheckForDuplicateMaterialId().then(() => SaveData());
      } else {
        SaveData();
      }
    });

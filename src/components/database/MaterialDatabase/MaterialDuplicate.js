...
const MaterialDuplicate = () => {
  ...
  // Load data on mount (copy from existing material but clear the ID)
  useEffect(() => {
    if (RowId) {
      ...
      if (material) {
        // Copy material data but clear the Material_Id for duplicate
        setSelectedMaterialData({ ...material, Material_Id: "" });

        // Set unit data from material data
        const unitData = {};
        Object.keys(material).forEach((key) => {
          unitData[key] = { value: key === "Material_Id" ? "" : material[key], unit_id: "" };
        });
  ...
  const SaveData = () => {
    ...
    if (SamePage) {
      const newRowId = btoa(newId);
      history.push(`/database/Options/${newRowId}/MaterialView`);
    } else {
  ...
  const CheckForDuplicateMaterialId = () => {
    return new Promise((resolve) => {
      ...
      const exists = StoredMaterialData.some(
        (m) => String(m.Material_Id).toLowerCase() === String(SelectedMaterialData["Material_Id"]).toLowerCase()
      );
  ...
  const addColumn = () => {
    if (!header.header) {
      ...
      const exists = column.some((h) => h.headerText?.toLowerCase() === header.header.toLowerCase());
      if (!exists) {
        const newColumn = {
          id: nanoid(),
    ...
    <>
      ...
      <div className="container-fluid">
        <div className="d-flex justify-content-between ml-3 pt-3 pb-3">
          <BreadCrumb DB_Name={"Material"} Current_Page={"Duplicate"} />
        </div>
...
export default MaterialDuplicate;

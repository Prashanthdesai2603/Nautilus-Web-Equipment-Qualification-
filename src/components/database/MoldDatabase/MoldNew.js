...
const MoldNew = () => {
  ...
  const [moldUnitData, setMoldUnitData] = useState({
    ...
    Hot_Runner_Volume: { value: "", unit_id: "" },
    Cycle_Time: { value: "", unit_id: "" },
    Mold_Stack_Height: { value: "", unit_id: "" },
    Mold_Vertical_Height: { value: "", unit_id: "" },
    Req_Mold_Open_Stroke: { value: "", unit_id: "" },
    Mold_Width: { value: "", unit_id: "" },
  ...
  const [ModalStates, setModalStates] = useState({
    MoldIdConfirm: {
      visibility: false,
      title: "Confirm Mold No",
      message: "Mold No is mandatory.",
    },
    MoldIdUnique: {
      visibility: false,
      title: "Duplicate Mold No",
      message: "Mold No should be unique. This Mold No already exists.",
    },
    ConvertConfirm: {
      visibility: false,
      title: "Confirm Unit Change",
      message: "You are changing the units, do you want to change the value accordingly?",
    },
  });
  ...
  const handleChange = (event) => {
    ...
    setMoldUnitData({
      ...moldUnitData,
      [name]: { value: value, unit_id: "" },
    });
  };

  const SaveData = () => {
    const StoredMoldData = JSON.parse(sessionStorage.getItem("MoldData")) || [];
    const newId = StoredMoldData.length > 0 
      ? Math.max(...StoredMoldData.map(m => m.id)) + 1 
      : 1;
  ...
  return (
    <>
      <Modal isOpen={ModalStates.MoldIdConfirm.visibility} centered={true} style={{ width: "400px" }}>
        ...
        <ModalFooter>
          <Button color="primary" onClick={() => ToggleModalStates("MoldIdConfirm")}>Close</Button>
        </ModalFooter>
      </Modal>
      ...
      <Modal isOpen={ModalStates.MoldIdUnique.visibility} centered={true} style={{ width: "400px" }}>
        ...
        <ModalFooter>
          <Button color="primary" onClick={() => ToggleModalStates("MoldIdUnique")}>Close</Button>
        </ModalFooter>
      </Modal>
              ...
              <div className="pt-2 pb-2">
                <label className="ml-3">
                  <input
                    type="checkbox"
                    checked={SamePage}
                    onChange={(e) => setSamePage(e.target.checked)}
                  />{" "}
                  View after save
                </label>
              </div>

...
const MachineNew = () => {
  ...
  const [ModalStates, setModalStates] = useState({
    MachineIdConfirm: {
      visibility: false,
      title: "Confirm Machine Number",
      message: "Machine Number is mandatory.",
    },
    MachineIdUnique: {
      visibility: false,
      title: "Duplicate Machine Number",
      message: "Machine Number should be unique. This Machine Number already exists.",
    },
  });
  ...
  const SaveData = () => {
    ...
    const newId = StoredMachineData.length > 0
      ? Math.max(...StoredMachineData.map(m => m.id)) + 1
      : 1;
  ...
  return (
    <>
      <Modal isOpen={ModalStates.MachineIdConfirm.visibility} centered={true} style={{ width: "400px" }}>
        ...
        <ModalFooter>
          <Button color="primary" onClick={() => ToggleModalStates("MachineIdConfirm")}>Close</Button>
        </ModalFooter>
      </Modal>
      ...
      <Modal isOpen={ModalStates.MachineIdUnique.visibility} centered={true} style={{ width: "400px" }}>
        ...
        <ModalFooter>
          <Button color="primary" onClick={() => ToggleModalStates("MachineIdUnique")}>Close</Button>
        </ModalFooter>
      </Modal>

      <div className="container-fluid">
        <div className="d-flex justify-content-between ml-3 pt-3 pb-3">
          <BreadCrumb DB_Name={"Machine"} Current_Page={"New"} />
        </div>
          ...
          <div className="card p-3 ml-2" style={{ background: "#e4eae1" }}>
            ...
            <div className="d-flex col-md-12">
              <div className="col-md-6">
                <LeftInputSection
                  machineUnitData={machineUnitData}
